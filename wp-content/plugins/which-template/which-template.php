...
License: GPL2
*/
namespace WhichTemplate;

add_action('wp_footer', 'WhichTemplate\which_template_included_files');
...
add_filter('template_include', 'WhichTemplate\which_template_template_include', 1000);
function which_template_template_include($template) {
  ...
  if (is_user_logged_in()) {
    ...
    add_filter('admin_bar_menu', 'WhichTemplate\which_template_admin_bar_menu', 1000);
  }
  return $template;
}
...
function which_template_admin_bar_menu($template) {
  ...
  if (is_user_logged_in()) {
    ...
    if (isset($GLOBALS["which_included_templates"])) {
